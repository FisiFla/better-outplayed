...
use config::{Config, StorageSection};
...
/// and — before this — kept forever. The clips directory therefore only grows when the
/// user presses the clip hotkey, and one pass costs a `list_clips` plus a `SUM`, i.e. a
/// few hundred microseconds on a library of thousands of rows. Five minutes is long
/// enough that the cost is invisible in a capture path that submits 30-60 frames a
/// second, and short enough that a session's clips cannot drift far past
/// `storage.max_total_bytes` before they are managed. A fresh run applies the policy at
/// startup, so anything a previous session left over is handled immediately rather than
/// five minutes in.
...
pub fn run_buffer() -> Result<()> {
    ...
    // The clip index (spec §5.5) comes up before anything is captured, and the storage
    // policy (spec §8.1) is applied immediately: whatever a previous session left in the
    // clips directory is managed now rather than five minutes into this one. Opening the
    // index is the one step here that fails hard — see [`open_clip_index`].
    let store = open_clip_index(&app_dir.join("localplay.db"))?;
    ...
    loop {
        ...
        if last_scan.elapsed() >= Duration::from_millis(200) {
            ...
            // Periodic storage-policy pass (spec §8.1), on the same 200ms tick as the
            // scratch scan that gates it. `cleanup_pass` is silent when there is nothing
            // to do, which is every pass in the ordinary case.
        ...
        if localplay_events::hotkey::wait_for_press(&hotkeys, Duration::from_millis(10)) {
            ...
            // `ledger_origin_ms` moves that onto the ledger's timeline, which is the
            // timeline segment numbering — and so the footage itself — is measured on.
            let started_at_ms =
                ring.ledger_origin_ms() + trigger_ms.saturating_sub(buffer_cfg.pre_ms);
            index_clip(&store, &clip, started_at_ms);
        }
    }
}
...
fn open_clip_index(db_path: &Path) -> Result<Store> {
...
/// * `session_id` is left NULL. The schema allows it, and this CLI does not open a
///   `sessions` row: the capture loop has no clean shutdown to stamp `ended_at` on, so a
///   session row could never be finished. NULL is the honest value; a made-up id would
///   point at a session that does not exist.
///
...
/// because it runs every [`CLEANUP_INTERVAL`] and an idle pass is not news.
fn cleanup_pass(store: &Store, storage: &StorageSection, report: &mut CleanupReport) {
...
mod tests {
    ...
    fn a_cleanup_pass_applies_the_configured_policy_through_the_index() {
        ...
        // Two clips of 1_000 bytes each, indexed exactly as the hotkey path indexes them.
